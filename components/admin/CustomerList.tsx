...
export const CustomerList: React.FC = () => {
 ...
 <div className="p-2.5 bg-background-subtle "><Icon name="people" className="text-xl text-primary " /></div>
 <div><p className="text-2xl font-bold text-foreground ">{customers.length}</p><p className="text-sm text-foreground-muted ">Total Customers</p></div>
 </div>
 </div>
 <div className="bg-background-card border border-border rounded-2xl shadow-2xl p-5">
 <div className="flex items-center gap-3">
 <div className="p-2.5 bg-green-900/20 bg-green-900/20"><Icon name="person_check" className="text-xl text-green-400 " /></div>
 <div><p className="text-2xl font-bold text-foreground ">{activeThisMonth}</p><p className="text-sm text-foreground-muted ">Active This Month</p></div>
 </div>
 </div>
 <div className="bg-background-card border border-border rounded-2xl shadow-2xl p-5">
 <div className="flex items-center gap-3">
 <div className="p-2.5 bg-primary/10"><Icon name="payments" className="text-xl text-primary " /></div>
 <div><p className="text-2xl font-bold text-foreground ">₹{totalRevenue.toLocaleString()}</p><p className="text-sm text-foreground-muted ">Lifetime Revenue</p></div>
 </div>
 </div>
 </div>
 ...
 <p className="text-foreground-muted text-sm">Loading customers…</p>
 </div>
 ) : (
 <div className="overflow-x-auto">
 <table className="w-full">
 <thead>
 <tr className="bg-background-card border-b border-border ">
 <th className="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider text-foreground-muted ">Customer</th>
 <th className="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider text-foreground-muted ">Orders</th>
 <th className="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider text-foreground-muted ">Total Spent</th>
 <th className="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider text-foreground-muted ">Last Order</th>
 </tr>
 </thead>
 ...
 <td className="py-4 px-4"><span className="font-medium text-foreground ">{customer.totalOrders}</span></td>
 <td className="py-4 px-4"><span className="font-semibold text-green-400 ">₹{customer.totalSpent.toLocaleString()}</span></td>
 <td className="py-4 px-4"><span className="text-foreground-muted ">{new Date(customer.lastOrderAt).toLocaleDateString()}</span></td>
 </tr>
 );
 })}
 </tbody>
 ...
 <Icon name="person_search" className="text-4xl text-foreground-muted mb-3" />
 <p className="text-foreground-muted ">{searchQuery ? 'No customers match your search' : 'No customers yet'}</p>
 </div>
 )}
 </div>
 )}
