        ...
        >
            ...
            <div className="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Customize your print options</p>
                </div>
                <button
                    onClick={reset}
            ...
            <div className="space-y-5 flex-1 overflow-y-auto overflow-x-hidden no-scrollbar">
                ...
                {/* SIDES */}
                <section>
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 block">Sides</label>
                    <div className="flex flex-wrap gap-2">
                        <OptionButton
                            label="Single"
                ...
                {/* PAPER SIZE */}
                <section>
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 block">Paper Size</label>
                    <div className="relative">
                        <select
                            value={options.paperSize}
                            onChange={(e) => update('paperSize', e.target.value as PrintOptions['paperSize'])}
                            className="w-full appearance-none px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.06] text-white text-sm font-medium focus:outline-none focus:border-white/20 transition-colors cursor-pointer"
                        >
                ...
                {/* COPIES */}
                <section>
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 block">Copies</label>
                    <div className="flex items-center gap-4">
                        <motion.button
                            whileTap={{ scale: 0.85 }}
                            onClick={() => update('copies', Math.max(1, options.copies - 1))}
                            className="size-10 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-white hover:bg-white/[0.08] transition-colors"
                        >
                            <Minus size={16} />
                        </motion.button>
                        ...
                        <motion.button
                            ...
                            className="size-10 rounded-xl bg-white text-black flex items-center justify-center hover:bg-gray-200 transition-colors"
                        >
                            <Plus size={16} />
                        </motion.button>
                    </div>
                ...
                <section>
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 block">Pages to Print</label>
                    <input
                        type="text"
                        value={options.pageRangeText}
                        onChange={(e) => update('pageRangeText', e.target.value)}
                        placeholder="e.g. All, 1-10, 5,8,12-20"
                        className="w-full px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] text-white text-sm focus:outline-none focus:border-white/20 transition-colors placeholder-white/20"
                    />
                    <p className="text-[10px] text-gray-600 mt-1.5">Leave blank to print all pages</p>
                </section>

                {/* BINDING */}
                <section>
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 block">Binding</label>
            ...
            <div className="hidden lg:block mt-6 pt-6 border-t border-white/[0.06]">
                <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Cost Breakdown</h3>
                <div className="space-y-1.5">
                    {breakdown.lines.map((line, i) => (
                        <div key={i} className="flex justify-between items-baseline text-sm">
                            <div className="flex items-baseline gap-1.5 min-w-0">
                                <span className={cn('truncate', line.amount < 0 ? 'text-emerald-400' : 'text-gray-500')}>{line.label}</span>
                                {line.detail && <span className="text-[10px] text-gray-600 shrink-0">{line.detail}</span>}
                            </div>
