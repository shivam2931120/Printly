...
export const ProfilePage: React.FC = () => {
 ...
 <div>
 <h2 className="text-2xl font-black text-foreground mb-2 font-display">Guest Session</h2>
 <p className="text-foreground-muted text-sm max-w-xs mx-auto leading-relaxed">
 Sign in to sync your orders across devices and access premium features.
 </p>
 </div>
 <Button
 onClick={() => navigate('/sign-in')}
 className="bg-primary text-foreground hover:bg-background-card/90 font-black px-8 h-12 transition-all"
 >
 ...
 <h3 className="text-xs font-bold text-foreground-muted uppercase tracking-widest pl-2">Recent Device Orders</h3>
 {orders.length === 0 ? (
 ...
 <Button variant="link" onClick={() => navigate('/')} className="text-foreground mt-2">
 Start Printing
 </Button>
 </div>
 ) : (
 ...
 <div key={order.id} className="p-5 flex items-center justify-between hover:bg-background-subtle transition-colors">
 <div className="flex items-center gap-4">
 <div className="size-10 bg-background-subtle flex items-center justify-center text-foreground shrink-0">
 <Package size={20} />
 </div>
 <div>
 <div className="flex items-center gap-2">
 <p className="font-bold text-foreground text-sm">Order #{order.id.slice(-6).toUpperCase()}</p>
 {order.otp && (
 <span className="px-2 py-0.5 bg-background-subtle text-primary text-[10px] font-bold rounded border border-primary/30 tracking-wider">
 OTP: {order.otp}
 </span>
 )}
 </div>
 ...
 {/* Header / Profile Info */}
 <div className="bg-background-card border border-border rounded-2xl shadow-2xl p-10 flex flex-col items-center text-center relative overflow-hidden">
 <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-white/10 to-transparent" />

 <div className="relative mb-6">
 <div className="size-28 bg-background-subtle p-1 border border-border ">
 <img
 ...
 <div className="absolute -bottom-2 -right-2 size-8 bg-primary text-foreground flex items-center justify-center border-4 border-[#050505]">
 <UserIcon size={14} fill="currentColor" />
 </div>
 </div>

 <div className="space-y-1 mb-6">
 <h1 className="text-3xl font-black text-foreground font-display tracking-tight">{user.name}</h1>
 <p className="text-foreground-muted text-sm font-medium opacity-60">{user.email}</p>
 </div>
 ...
 className="flex flex-col items-start gap-4 p-6 bg-background-card border border-border rounded-2xl shadow-2xl hover:bg-background-subtle transition-all group"
 >
 <div className="size-12 bg-background-subtle flex items-center justify-center text-foreground group-hover:scale-110 transition-transform">
 <Package size={24} />
 </div>
 <div>
 <p className="font-black text-foreground text-sm uppercase tracking-wider">Orders</p>
 <p className="text-[10px] text-foreground-muted font-bold opacity-40 uppercase tracking-widest mt-1">Track history</p>
 </div>
 </button>
 ...
 <div>
 <p className="font-black text-foreground text-sm uppercase tracking-wider">Contact</p>
 <p className="text-[10px] text-foreground-muted font-bold opacity-40 uppercase tracking-widest mt-1">Get assistance</p>
 </div>
 </button>
 </div>
 ...
 <div className="space-y-4">
 <h3 className="text-xs font-bold text-foreground-muted uppercase tracking-widest pl-2">Recent Orders</h3>
 {orders.length === 0 ? (
 <div className="bg-background-card border rounded-2xl shadow-xl border-border p-8 text-center">
 <p className="text-foreground-muted">No orders yet.</p>
 <Button variant="link" onClick={() => navigate('/')} className="text-foreground mt-2">
 ...
 {/* Settings & Logout */}
 <div className="space-y-4">
 <h3 className="text-[10px] font-black text-foreground-muted uppercase tracking-[0.2em] pl-4 opacity-50">Account Control</h3>
